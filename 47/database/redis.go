...
func (c *Cacher) Exsist(name string) error {
	if c.rdb.Exists(context.Background(), c.prefix+"_"+name).Val() > 0 {
		return nil
	}
...
func (c *Cacher) Incr(name string) (int64, error) {
	result := c.rdb.Incr(context.Background(), c.prefix+"_"+name).Val()
	if result > 0 {
		return result, nil
	}
	return 0, c.rdb.Incr(context.Background(), c.prefix+"_"+name).Err()
}
